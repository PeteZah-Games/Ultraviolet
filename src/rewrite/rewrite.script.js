...
function property(ctx) {
  ...
  js.on('MemberExpression', (node, data, type) => {
    ...
    if (
      (!node.computed && node.property.name === 'location' && type === 'rewrite') ||
...
function identifier(ctx) {
  ...
  js.on('Identifier', (node, data, type) => {
    ...
    if ((parent.type === 'AssignmentExpression' || parent.type === 'AssignmentPattern') && parent.left === node) return false;
    if ((parent.type === 'FunctionExpression' || parent.type === 'FunctionDeclaration') && parent.id === node) return false;
    if (parent.type === 'MemberExpression' && parent.property === node && !parent.computed) return false;
    if (node.name === 'eval' && parent.type === 'CallExpression' && parent.callee === node) return false;
    if (parent.type === 'Property' && parent.key === node) return false;
    if (parent.type === 'Property' && parent.value === node && parent.shorthand) return false;
    if (parent.type === 'UpdateExpression' && (parent.operator === '++' || parent.operator === '--')) return false;
    if (
      (parent.type === 'FunctionExpression' || parent.type === 'FunctionDeclaration' || parent.type === 'ArrowFunctionExpression') &&
...
function importDeclaration(ctx) {
  ...
  js.on('Literal', (node, data, type) => {
    if (
      !(
        (node.parent.type === 'ImportDeclaration' || node.parent.type === 'ExportAllDeclaration' || node.parent.type === 'ExportNamedDeclaration') &&
...
function unwrap(ctx) {
  ...
  js.on('CallExpression', (node, data, type) => {
    ...
    switch (node.callee.property.name) {
      case '$wrap':
        {
          if (!node.arguments || node.parent.type !== 'MemberExpression' || node.parent.property !== node) return false;
...
function isWrapped(node) {
  ...
  if (node.object.type !== 'Identifier' || node.object.name !== '__uv') return false;
...
export { dynamicImport, identifier, importDeclaration, importMeta, property, unwrap, wrapEval };
