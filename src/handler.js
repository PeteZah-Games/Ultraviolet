...
function __uvHook(window) {
  ...
  client.idb.on('idbFactoryName', (event) => {
    event.data.value = event.data.value.slice(__uv.meta.url.origin.length + 1 /*the @*/);
  });
  ...
  const contentWindowGet = HTMLIFrameElement && Object.getOwnPropertyDescriptor(HTMLIFrameElement.prototype, 'contentWindow').get;
